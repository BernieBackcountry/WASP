...
def create_description() -> html.P:
    return html.P('The Webscraping Application for Satellite Pairing (WASP) consolidates pertinent'
                  'information to the satellite pair-building process. Enter a satellite below to obtain its related data.')
...
def create_button_celestrak() -> html.Div:
    return html.Div(className='button_celestrak', children=[
        ...
        html.Button('Update Celestrak TLEs', id='button-update-celestrak', n_clicks=0, 
                    style={'marginLeft': '5px', 'background-color': '#00263A', 'color': '#DBE2E9'})])
...
def create_contact_information() -> html.Footer:
    return html.Footer(className='footer', children=
                       'All information presented on this dashboard is aggregated and compiled from open-source, unclassified websites.'
                       'The information is copyrighted and not approved for commercial or public use. The original sources for this'
                       'information are https://www.satbeams.com/, https://www.lyngsat.com/, https://celestrak.org/, and'
                       'http://frequencyplansatellites.altervista.org/. For more information or questions, please contact'
                       'michelle.mcgee.2@spaceforce.mil and/or alexis.denhard.ctr@spaceforce.mil', 
                       style={'font-style': 'italic'})


def create_column_filter() -> dcc.Dropdown:
    return dcc.Dropdown(['Channel Status', 'Ku/C-band'], id='column-filter', placeholder='Filter by...', 
                        style={'marginLeft': '5px', 'maxWidth': '300px'})


def create_value_filter() -> dcc.Dropdown:
    return dcc.Dropdown(id='value-filter', style={'marginLeft': '5px', 'maxWidth': '300px'})
...
def create_data_table(df: pd.DataFrame) -> dash_table.DataTable:
    return dash_table.DataTable(df.to_dict('records'), [{'name': i, 'id': i} for i in df.columns],
                                ...
                                style_data_conditional=[{'if': {'column_id': 'Ku/C-band',},'display': 'None',}],
                                style_header_conditional=[{'if': {'column_id': 'Ku/C-band',},'display': 'None',}])
